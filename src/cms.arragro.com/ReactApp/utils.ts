...
export const isValidPhoneNumber = /^\+?[0-9 ]{7,13}$/i
...
export const contactYup = Yup.object().shape({
    ...
    linkedIn: Yup.string().nullable(true).max(255, 'LinkedIn has a 255 character limit.').matches(isValidUrl, { message: 'Please supply a valid URL', excludeEmptyString: true }),
...
export const tileYup = Yup.object().shape({
    ...
    href: Yup.string().nullable(true).default('').max(2000, 'Href has a 2000 character limit.').matches(isUrlRoute, { excludeEmptyString: true }),
    markdown: Yup.string().nullable(true).max(4000, 'Markdown has a 4000 character limit.'),
    cssClass: Yup.string().nullable(true).max(30, 'CSS Class has a 30 character limit.')
})
...
export const svgIconLinkYup = Yup.object().shape({
    ...
    href: Yup.string().nullable(true).max(2000, 'Href has a 255 character limit.').matches(isUrlRoute, { message: 'Please supply a valid url route e.g. "home".', excludeEmptyString: true }),
    markdown: Yup.string()
})
...
export const carouselYup = Yup.object().shape({
    ...
    imageAlt: Yup.string().max(512, 'Image Alt has a 512 character limit.'),
    linkText: Yup.string(),
    href: Yup.string().max(2000, 'Href has a 2000 character limit.').matches(isUrlRoute)
})
